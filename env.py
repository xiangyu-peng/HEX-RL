...
GraphInfo = collections.namedtuple('GraphInfo','objs, ob_rep, act_rep, graph_state, graph_state_rep, graph_state_1, graph_state_rep_1, graph_state_2, graph_state_rep_2, graph_state_3, graph_state_rep_3, graph_state_4, graph_state_rep_4, graph_state_5_mask, graph_state_rep_5_mask, admissible_actions, admissible_actions_rep')
...
class QBERTEnv:
    ...
    '''
    def __init__(self, rom_path, seed, spm_model, tsv_file, attr_file, step_limit=None, stuck_steps=10, gat=True, askbert=False, clear_kg=False, subKG_type='SHA'):
        random.seed(seed)
        np.random.seed(seed)
    ...
    def _build_graph_rep(self, action, ob_r):
        ...
        return GraphInfo(
                        ...
                        graph_state_1=self.state_rep.graph_state_1,
                        graph_state_rep_1=self.state_rep.graph_state_rep_1,
                        graph_state_2=self.state_rep.graph_state_2,
                        graph_state_rep_2=self.state_rep.graph_state_rep_2,
                        graph_state_3=self.state_rep.graph_state_3,
                        graph_state_rep_3=self.state_rep.graph_state_rep_3,
                        graph_state_4=self.state_rep.graph_state_4,
                        graph_state_rep_4=self.state_rep.graph_state_rep_4,
    ...
    def reset(self):
        self.state_rep = StateAction(spm=self.spm_model,
                                     vocab=self.vocab,
                                     vocab_rev=self.vocab_rev,
                                     tsv_file=self.tsv_file,
                                     max_word_len=self.max_word_len,
                                     askbert=self.askbert,
                                     attr_file=self.attr_file,
                                     subKG_type=self.subKG_type)
        self.stuck_steps = 0
        self.valid_steps = 0
        self.episode_steps = 0
    ...
    def get_valid_actions(self, ob_r):
        res = ''
        for ob in ob_r:
            res += ob + ' '
        objs = [o[0] for o in self.env.identify_interactive_objects(res)]
        objs.append('all')
        admissible_actions = self._get_admissible_actions(objs)
        admissible_actions_rep = [self.state_rep.get_action_rep_drqa(a.action) \
                                  for a in admissible_actions] \
            if admissible_actions else [[0] * 20]
        print('admissible_actions_rep', admissible_actions)
        return admissible_actions_rep
