...
class Extraction(object):
    ...
    def generate(self, input_text, threshold=0.2, attribute=True):
        ...
        if attribute:
            ...
            # print('self.extract_entity(input_text, preds, probs, threshold/10)', self.extract_entity(input_text, preds, probs, 0.05))
        return {'location': locs, 'object_surr': objs_surr, 'objs_inv': objs_inv, 'attributes': attributes}

if __name__ == '__main__':
    ex = Extraction()
    print(ex.generate(input_text='This is a far from luxurious but still quite functional bathroom, with a sink, toilet and shower. '))
    print(ex.generate(input_text='The bedroom lies to the north.'))
    print(ex.generate(input_text='I love California'))